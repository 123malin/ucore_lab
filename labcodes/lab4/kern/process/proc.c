...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
    ...
    proc->parent = current;                 //create a process which is the child process of the current
    
    if (setup_kstack(proc) != 0) {          //allocate a kernel stack for child process
        goto bad_fork_cleanup_proc;
    }
    if (copy_mm(clone_flags, proc) != 0) {  //dup or share mm according chone_flag
        goto bad_fork_cleanup_kstack;
    }
    copy_thread(proc, stack, tf);           //setup tf and context in proc_struct
    ...
    wakeup_proc(proc);                    // make the new child process RUNNABLE

    ret = proc->pid;                      //set return value being child process's pid
fork_out:
    return ret;
